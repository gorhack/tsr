...
grep -q "JAVA_HOME" ${profileHome}
if [[ $? != 0 ]]; then
  printf "\n\nJAVA_HOME is not set: Setting JAVA_HOME\n\n"
  if [[ ! $(echo "$JAVA_HOME") =~ .*"jdk-13".* ]]; then
    printf "JAVA_HOME previously set to an incorrect version, you may have to remove previous PATH from $profileHome"
  fi
  printf "\n\nSetting JAVA_HOME\n\n"
  if [[ ${operatingSystem} == "Darwin" ]]; then
 ...
 installOpenJDKMac() {
    ...
    printf "\n\nInstalling OpenJDK13...\n\n"
    brew cask install adoptopenjdk13
 }

 installOpenJDKUbuntu() {
    printf "Downloading OpenJDK13 tar"
    curl https://download.java.net/openjdk/jdk13/ri/openjdk-13+33_linux-x64_bin.tar.gz --output openjdk-13+33_linux-x64_bin.tar.gz
    sudo mkdir -p /usr/java
    sudo tar -xzf openjdk-13+33_linux-x64_bin.tar.gz -C /usr/java
...
if [[ ! $(which java) ]]; then
    ...
    if [[ ${JAVA_VER} != "13" ]]; then
          printf "\n\nFound Java installation with incorrect version: Installing OpenJDK13!\n\n"
          if [[ ${operatingSystem} == "Linux" ]]; then
                installOpenJDKUbuntu
          elif [[ ${operatingSystem} == "Darwin" ]]; then
                installOpenJDKMac
